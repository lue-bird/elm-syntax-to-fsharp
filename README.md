...
-   only a subset of elm is currently supported. not supported:
    ...
    -   potential future candidates: `List.map5`, `List.map4`, `Bitwise.shiftRightZfBy`, `Array` (and potentially an optimized `Set`) Any help appreciated!
-   no checks are performed before transpiling to F#. So if you don't add a compile check of your elm input,
